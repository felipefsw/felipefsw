...
3. Selecione este repositório e clique em **Import** (ainda **não** clique em Deploy).
4. Crie um banco Postgres e pegue **duas** connection strings:
   - **Vercel Postgres:** aba **Storage → Create Database → Postgres**.
   - **Supabase/Neon:** crie o projeto e copie as strings em *Connection settings*.
   Você precisa da **pooled** (com pgbouncer) e da **direta** (non-pooling).
5. Em **Settings → Environment Variables** do projeto, adicione:
   - `DATABASE_URL` = connection string **pooled**
   - `DIRECT_URL` = connection string **direta** (non-pooling)
6. Clique em **Deploy**. No build, as tabelas são criadas sozinhas (`prisma migrate deploy`
   usa a `DIRECT_URL`).
7. Pronto! A Vercel te dá um endereço (ex.: `seu-app.vercel.app`). Abra no celular e use
...
rode o seed privado apontando a `DATABASE_URL`/`DIRECT_URL` para o banco da Vercel:

```bash
DATABASE_URL="<pooled>" DIRECT_URL="<direta>" npx tsx prisma/seed.real.ts
```
